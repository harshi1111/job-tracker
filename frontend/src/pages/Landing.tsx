...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        // Get total applications count from all users
  ...
  // Also need to add a backend endpoint for stats
  // Create backend/src/routes/stats.routes.ts and controller
    ...
    <div className={`min-h-screen transition-colors duration-300 overflow-x-hidden relative ${
      ...
      >
        <div className="text-center max-w-5xl mx-auto relative z-10">
          ...
          >
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center shadow-2xl shadow-cyan-500/25">
              <span className="text-3xl font-bold text-white">P</span>
            </div>
            <span className={`text-4xl font-bold tracking-tight ${
              theme === 'dark' ? 'text-cyan-400' : 'bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent'
            }`}>
              PATHGRID
            </span>
          </motion.div>
