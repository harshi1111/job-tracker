...
// Create provider instances with their keys
interface GroqProvider {
  name: string;
  client: Groq;
  key: string;
}
...
const groqProviders: GroqProvider[] = groqKeys.map((key, index) => ({
  name: `Groq-${index + 1}`,
  client: new Groq({ apiKey: key }),
  key: key
}));

const geminiProviders: GeminiProvider[] = geminiKeys.map((key, index) => ({
  name: `Gemini-${index + 1}`,
  client: new GoogleGenerativeAI(key),
  key: key
}));

const openrouterProviders: OpenRouterProvider[] = openrouterKeys.map((key, index) => ({
...
function getNextGroqProvider(): GroqProvider | null {
  if (groqProviders.length === 0) return null;
...
function getNextGeminiProvider(): GeminiProvider | null {
  if (geminiProviders.length === 0) return null;
...
function getNextOpenRouterProvider(): OpenRouterProvider | null {
  if (openrouterProviders.length === 0) return null;
...
export function resetProviderIndices() {
  currentGroqIndex = 0;
...
export const parseJobDescriptionStream = async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (let i = 0; i < openrouterProviders.length * 2; i++) {
      ...
      try {
        ...
        if (jsonMatch) fullResponse = jsonMatch[0];
...
export const generateResumeSuggestionsStream = async (req: Request, res: Response) => {
